...
fn font_encoding_differences_override() {
    ...
    let enc = FontEncoding::from_standard_with_differences(StandardEncoding::WinAnsi, &differences);
...
fn resolver_default_only() {
    let resolver = EncodingResolver::new(FontEncoding::from_standard(StandardEncoding::WinAnsi));
