...
fn stats_block_counts_are_consistent() {
    ...
    assert_eq!(
        total_from_pages, total_from_stats,
...
fn sections_cover_all_blocks() {
    ...
    assert_eq!(
        section_block_count + heading_count,
        total_from_pages,
...
fn headings_font_size_above_zero() {
    ...
    for h in doc.headings() {
        assert!(
            h.font_size > 0.0,
            "heading font_size should be > 0, got {}",
            h.font_size
        );
    }
}
...
fn document_text_is_nonempty_for_text_pdf() {
    ...
    assert!(
        !text.trim().is_empty(),
        "document text should not be empty for a text PDF"
    );
}
...
fn no_tables_when_disabled() {
    ...
    assert_eq!(
        ...
        0,
        "tables should be 0 when detection disabled"
    );
}
...
fn no_figures_when_disabled() {
    ...
    let doc = Document::from_pdf_with_options(&pdf, &opts);
    assert_eq!(
        doc.stats().figure_count,
        0,
        "figures should be 0 when detection disabled"
    );
}
...
fn page_layout_accessors_consistent_with_blocks() {
    ...
    for page in doc.pages() {
        ...
        assert_eq!(
            total,
            page.blocks.len(),
            "accessor counts don't sum to block count"
        );
    }
}
...
fn to_markdown_is_nonempty_for_text_pdf() {
    ...
    assert!(
        !md.trim().is_empty(),
        "markdown should not be empty for a text PDF"
    );
}
...
fn to_markdown_contains_heading_syntax() {
    ...
    if doc.stats().heading_count > 0 {
        assert!(
            md.contains('#'),
            "markdown should contain ATX headings when headings detected"
        );
    }
}
...
fn block_to_markdown_headings() {
    ...
    use pdfplumber_layout::{Heading, HeadingLevel, LayoutBlock, block_to_markdown};
