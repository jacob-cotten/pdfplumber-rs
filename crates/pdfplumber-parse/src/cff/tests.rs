...
fn build_cff_data(
    ...
    let (top_dict_index, charstrings_offset_final, private_offset_final) =
        if actual_cs_offset != cs_offset_est || actual_priv_offset != priv_offset_est {
    ...
    let (top_dict_index, _cs_off, _priv_off) = {
        ...
        if verify_cs != charstrings_offset_final || verify_priv != private_offset_final {
            ...
            let cs3 =
                header_size + name_size + tdi3.len() + string_index.len() + global_subr_index.len();
